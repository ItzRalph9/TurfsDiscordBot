...
intents.all()
bot = commands.Bot(command_prefix="!", intents=intents, help_command=None, activity=discord.Game(name="Gathering Turfs"))
...
def AddNewUser(gang_name, gang_id, name, id):
    ...
    with open(f"{file_path}") as json_file:
        ...
        data_to_write = {"name" : f"{name}", "money" : 0, "turfs" : 0}
...
def UpdateUndoFile(ctx):
    ...
    copy_command = str(f'cp "{file_name}" "{undo_file_name}"') # CHANGE "copy" TO "cp" IN LINUX IF IT SAYS LIKE SH: CAN'T COPY FILE
    os.system(copy_command)

def UpdateValues(gang_name, gang_id, id, name, turf_value, amount_of_people, ctx):
    CheckPaths(ctx)
    if IdInFile(id, gang_name, gang_id) == False:
        AddNewUser(gang_name, gang_id, name, id)
...
async def on_message(message):
    ...
    if ids == []:
        ...
        for id in ids:
            name = await GetName(id)
            names.append(name)
            UpdateValues(server_name, server_id, id, name, turf_value, amount_of_people, message)
        names = NameLayoutMentions(names)
        await message.channel.send(f"The turf has been saved!\nTurf value: **{turf_value_for_message}**\nYou tagged **{len(ids)}** people: {names}")

# Token for LinuxBot
LinuxBot_token = "Yeah im not giving you my token"
# Token for Auto Turf Bot
AutoTurfBot_token = "Same here"

bot.run(AutoTurfBot_token)